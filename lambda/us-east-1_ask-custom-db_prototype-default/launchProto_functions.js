...
//arrayのkeyをシャッフルして返す
function objeckKeyShuffled(array){
...
//仮想配列のkeyをシャッフルするとき
function objectKeyShuffled(object){
  return shuffle(Object.keys(object));
}


module.exports.hasEmptyContentInMyself = function(persistentMemory){
  for(var i=0 ; i<Object.keys(persistentMemory['myself']).length ; i++){
    if(persistentMemory['myself'][i]['response'].length == 0){
      return i;
    }
  }
  return -1;
...
module.exports.setGenre = function(genreParam){
  //myselfのときはmyselfを返す
  if(genreParam === 'myself') return 'myself';
  //myselfでないとき
  else{
    ...
    for(key in genreParam){
        if(key === 'family' || key === 'lifestyle' || key === 'assets'){
          ...
          for(var i=0; i<genreParam[key].length; i++){
            if(genreParam[key][i].length == 0) return key;
          }
        }
    }
...
module.expoets.getQuestion = function(persistentMemory,genre,item){
